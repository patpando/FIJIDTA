...
for (ii = 0; ii < fileList.length; ii++) { // For loop runs through all files/ images in chosen directory/ dir
	...
	if (sortCoord == true) {
		coorArray = sortCoorArray(coorArray); //Sort coordinates from left to right rather than by the order they were marked in
	}
	...
	for (i = 0; i < coorArray.length; i = i + 3) { // For loop will run as many times as there were overlay selections in the image. +3 for X Y and Z
		...
		if (onlyUseUserMark == true) {
			...
			if (newAreaInfo[3] < circularity) {
				quality = "B";
			} else if (newAreaInfo[2] < minAreaSizeum) {
				quality = "B";
			} else if (newAreaInfo[4] == true) {
				quality = "B";
			} else if (newAreaInfo[5] == false) {
				quality = "B";
			} else if (newAreaInfo[6] == false) {
				quality = "B";
			} else {
				quality = "G";
			}
...
function graphArea() {
	...
	if (threshSweep == true) {
		...
		print("FIJIDTAcc: " + totalCount + " / " + ALLEBs + " Average area: " + areaMean + " Average intensity: " + intMean);
	}
	if (printStats == true) {
